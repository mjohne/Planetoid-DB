...
/// restricted three-body problem. It is defined as:
/// <para><c>T_P = a_P / a + 2 * cos(i) * sqrt(a / a_P * (1 - e²))</c></para>
/// where <c>a_P</c> is the semi-major axis of the reference planet, <c>a</c> is the semi-major axis of the minor
/// planet, <c>e</c> is the eccentricity of the minor planet, and <c>i</c> is the orbital inclination of the minor
/// planet. By convention <c>T_J</c> (relative to Jupiter) is the most commonly used form and is widely employed
/// to classify small solar-system bodies.</remarks>
internal class TisserandParameterCalculator
{
	...
	/// <remarks>Values near 3 (relative to Jupiter) indicate a Jupiter-family comet or Jupiter-crossing orbit.
	/// Values greater than 3 typically indicate an asteroid, while values less than 2 suggest a nearly isotropic comet.</remarks>
	...
	/// <remarks>Values are taken from the standard IAU/JPL mean orbital elements
	...
	[
		new(Name: "Mercury", SemiMajorAxis: 0.387098),
		new(Name: "Venus",   SemiMajorAxis: 0.723332),
		new(Name: "Earth",   SemiMajorAxis: 1.000000),
		new(Name: "Mars",    SemiMajorAxis: 1.523679),
		new(Name: "Jupiter", SemiMajorAxis: 5.202603),
		new(Name: "Saturn",  SemiMajorAxis: 9.537070),
		new(Name: "Uranus",  SemiMajorAxis: 19.19126),
		new(Name: "Neptune", SemiMajorAxis: 30.06896),
	];
	...
	/// <remarks>Uses the standard three-body Tisserand formula:
	/// <para><c>T_P = a_P / a + 2 * cos(i) * sqrt(a / a_P * (1 - e²))</c></para>
	/// The result is undefined (and returned as <see cref="double.NaN"/>) when <paramref name="semiMajorAxis"/> is
	/// zero or negative.</remarks>
		...
		double inclinationDeg)
	{
		...
		double cosInclination = Math.Cos(inclinationRad);
		...
		foreach (PlanetData planet in Planets)
		{
			...
			double tisserand = planet.SemiMajorAxis / semiMajorAxis
				+ 2.0 * cosInclination * Math.Sqrt(semiMajorAxis / planet.SemiMajorAxis * oneMinusESq);
