...
public partial class DistributionsForm : BaseKryptonForm
{
	...
	/// <summary>Initializes a new instance of the <see cref="DistributionsForm"/> class.</summary>
	...
	public DistributionsForm(IReadOnlyList<string> planetoids)
	{
		InitializeComponent();
	...
	private void DistributionsForm_FormClosing(object? sender, FormClosingEventArgs e)
	{
		logger.Warn(message: "Form is closing, canceling any running histogram operation.");
	...
	private async void ToolStripButtonStartCancel_Click(object? sender, EventArgs e)
	{
		...
		{
			logger.Warn(message: "Cancellation requested for the running histogram generation task.");
		...
		if (_planetoids.Count == 0)
		{
			...
			logger.Error(message: "No planetoid data available for histogram generation.");
		...
		{
			logger.Error(message: "Histogram generation cannot proceed because either the orbital element or step size is not selected.");
		...
		catch (OperationCanceledException ex)
		{
			...
			logger.Info(exception: ex, message: "Orbit elements histogram generation was canceled by the user.");
	...
	private void ToolStripComboBoxOrbitElement_SelectedIndexChanged(object? sender, EventArgs e)
	{
		...
		{
			logger.Warn(message: "No valid histogram definition selected; clearing step-size options and resetting displayed results.");
		...
		{
			logger.Warn(message: "Step-size options populated for the selected histogram definition; selecting the first option by default.");
	...
	private void ToolStripComboBoxStepSize_SelectedIndexChanged(object? sender, EventArgs e)
	{
		logger.Info(message: "Step-size selection changed.");
		UpdateHistogramPlot(definition: GetSelectedDefinition(), results: _currentResults);
	}
	...
	{
		logger.Info(message: "Live-display setting changed.");
	...
	{
		logger.Info(message: "Log-scale setting changed.");
