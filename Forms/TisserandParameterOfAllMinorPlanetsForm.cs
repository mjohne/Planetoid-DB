...
public partial class TisserandParameterOfAllMinorPlanetsForm : BaseKryptonForm
{
	...
	protected override string ExportTitle => "Tisserand parameters of all minor planets";
	...
	protected override string ExportFilePrefix => "TisserandParametersOfAllMinorPlanets";
	...
	/// <summary>The complete list of Tisserand parameter results after the last completed calculation.</summary>
	/// <remarks>This list is only updated on the UI thread after the background calculation finishes.</remarks>
	private List<TisserandRowResult> _results = [];
	...
	/// <summary>Initializes a new instance of the <see cref="TisserandParameterOfAllMinorPlanetsForm"/> class.</summary>
	...
	/// <summary>Processes one raw MPCORB database record and appends the Tisserand parameter result row to <paramref name="results"/>.</summary>
	/// <param name="line">The raw MPCORB record string.</param>
	/// <param name="results">The list to which matching <see cref="TisserandRowResult"/> items are appended.</param>
	/// <remarks>Lines that are too short or have invalid orbital elements are silently skipped.</remarks>
	private static void ProcessPlanetoidLine(string line, List<TisserandRowResult> results)
	{
	...
	/// <remarks>Column 0 (Planetoid) is sorted as a string; all other columns (Tisserand parameter values) are sorted numerically.</remarks>
	private void SortResults()
	{
		...
		else if (col is >= 1 and <= PlanetCount)
		{
			...
			_results = ascending
				? [.. _results.OrderBy(keySelector: r => r.TisserandValues[planetIndex])]
				: [.. _results.OrderByDescending(keySelector: r => r.TisserandValues[planetIndex])];
		}
	}
	...
	private async void ButtonStart_Click(object sender, EventArgs e)
	{
		...
		List<TisserandRowResult> localResults = [];
