...
/// <summary>Represents a Windows Form that calculates and displays the Minimum Orbit Intersection Distance (MOID)
/// between two minor planets selected by the user from the loaded database.</summary>
/// <remarks>The form presents two drop-down combo boxes — one for each minor planet — populated with all
/// planetoid designations from the loaded database. A random-selection button sits next to each combo box.
/// The MOID result is shown in a label below the combo boxes and is recalculated automatically whenever
/// the user changes the selection in either combo box. The form does not appear in the taskbar and is
/// intended to be used as a modal dialog.</remarks>
