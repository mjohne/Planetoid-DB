...
public partial class OrbitsForm : BaseKryptonForm
{
	...
	/// <summary>Initializes a new instance of the <see cref="OrbitsForm"/> class.</summary>
	...
	public OrbitsForm(string planetoidName, double semiMajorAxis, double eccentricity, double argumentPerihelionDeg)
	{
		InitializeComponent();
		...
		logger.Info(
			message: "OrbitsForm initialized for planetoid '{0}' (a={1} AU, e={2}, ω={3}°).",
	...
	private void RenderOrbitPlot()
	{
		...
		var sun = formsPlotOrbits.Plot.Add.Circle(xCenter: 0.0, yCenter: 0.0, radius: SunRadiusAu);
		...
		{
			var (name, a, e, omega) = PlanetOrbits[i];
			AddOrbitEllipse(
				name: name,
				semiMajorAxis: a,
				eccentricity: e,
	...
	/// <remarks>
	/// <para>In the Keplerian two-body problem the Sun occupies one focus of the ellipse. To place
	/// the Sun at the origin the ellipse center must be displaced from the Sun by the linear
	/// eccentricity <c>c = a·e</c> in the direction opposite to perihelion.</para>
	/// <para>With the perihelion direction at angle <c>ω</c> from east, the center offset is:
	/// <c>cx = −a·e·cos(ω)</c>, <c>cy = −a·e·sin(ω)</c>.</para>
	/// <para>ScottPlot's <c>Ellipse.Rotation</c> property takes a <see cref="ScottPlot.Angle"/> value
	/// measured counter-clockwise from the positive X-axis (east), which matches the convention used
	/// here for the argument of perihelion.</para>
	/// <para>The ellipse fill is left transparent; only the border is colored so orbit lines
	/// remain visually distinct and do not occlude one another.</para>
	/// </remarks>
		...
		float lineWidth)
	{
		...
		double b = semiMajorAxis * Math.Sqrt(1.0 - eccentricity * eccentricity);
		...
		double cx = -c * Math.Cos(omegaRad);
		double cy = -c * Math.Sin(omegaRad);
		var ellipse = formsPlotOrbits.Plot.Add.Ellipse(xCenter: cx, yCenter: cy, radiusX: semiMajorAxis, radiusY: b);
