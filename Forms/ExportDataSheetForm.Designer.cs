/*
 * File:        ExportDataSheetForm.Designer.cs
...
/// <summary>Represents a form that enables users to export data sheets with selectable orbital elements and multiple file format options.</summary>
/// <remarks>The ExportDataSheetForm provides a user interface for choosing which orbital elements to include in an export and supports exporting to a variety of formats, such as text, LaTeX, Markdown, Word, Excel, CSV, HTML, XML, JSON, and others. The form is designed for use within the Planetoid-DB application and is accessible as a dialog. Controls are organized to facilitate marking and unmarking all elements, and export actions are available via a toolbar and context menu.</remarks>
