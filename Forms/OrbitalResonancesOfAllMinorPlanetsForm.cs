...
/// <summary>
/// Form for finding orbital resonances of all minor planets relative to the 8 known solar system planets.
/// </summary>
/// <remarks>
/// This form iterates over all planetoids in the database and computes their orbital resonances
/// with each selected planet. Results are displayed in a VirtualMode ListView.
/// The user can select which planets to include, start and cancel the search at any time.
/// </remarks>
...
public partial class OrbitalResonancesOfAllMinorPlanetsForm : BaseKryptonForm
{
	...
	/// <summary>
	/// Holds the list of all planetoid database strings passed to this form.
	/// </summary>
	/// <remarks>
	/// Each string is one raw MPCORB record from the database.
	/// </remarks>
	private readonly IReadOnlyList<string> _planetoids;

	/// <summary>
	/// Stores the computed resonance results to back the VirtualMode ListView.
	/// </summary>
	/// <remarks>
	/// Replaced atomically on the UI thread after each search completes.
	/// </remarks>
	private List<ResonanceResult> _results = [];

	/// <summary>
	/// Cancellation token source for the currently running search task.
	/// </summary>
	...
	/// <summary>
	/// Represents a single resonance result combining a planetoid designation with its resonance data.
	/// </summary>
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="OrbitalResonancesOfAllMinorPlanetsForm"/> class.
	/// </summary>
	/// <param name="planetoids">The list of all planetoid database records to process.</param>
	/// <remarks>
	/// Each element in <paramref name="planetoids"/> must be a raw MPCORB-format string.
	/// </remarks>
	...
	/// <summary>
	/// Returns the list of planet names currently selected via the planet checkboxes.
	/// </summary>
	/// <returns>A list of planet name strings that are checked.</returns>
	/// <remarks>
	/// Planet names match those used in <see cref="DerivedElements.CalculateOrbitalResonances"/>.
	/// </remarks>
	private List<string> GetSelectedPlanets()
	{
		List<string> selected = [];
		if (checkBoxMercury.Checked) selected.Add(item: "Mercury");
		if (checkBoxVenus.Checked) selected.Add(item: "Venus");
		if (checkBoxEarth.Checked) selected.Add(item: "Earth");
		if (checkBoxMars.Checked) selected.Add(item: "Mars");
		if (checkBoxJupiter.Checked) selected.Add(item: "Jupiter");
		if (checkBoxSaturn.Checked) selected.Add(item: "Saturn");
		if (checkBoxUranus.Checked) selected.Add(item: "Uranus");
		if (checkBoxNeptune.Checked) selected.Add(item: "Neptune");
		return selected;
	}

	/// <summary>
	/// Updates the progress bar value and text label.
	/// </summary>
	/// <param name="percent">Progress value from 0 to 100.</param>
	/// <remarks>
	/// The percentage is displayed both in the progress bar's <c>Text</c> property and in the adjacent label.
	/// </remarks>
	...
	/// Clears the status bar on startup.
	/// </summary>
	...
	/// Cancels any running search and disposes the cancellation token source.
	/// </summary>
	/// <param name="sender">Event source (the form).</param>
	...
	/// <summary>
	/// Handles the RetrieveVirtualItem event for the VirtualMode ListView.
	/// Provides the <see cref="ListViewItem"/> for the requested index from <see cref="_results"/>.
	/// </summary>
	...
	/// <remarks>
	/// Called by the ListView for each visible row. Must be fast and must not modify <see cref="_results"/>.
	/// </remarks>
	private void ListView_RetrieveVirtualItem(object sender, RetrieveVirtualItemEventArgs e)
	{
		...
		ResonanceResult result = _results[e.ItemIndex];
	...
	/// <summary>
	/// Handles the Click event of the Start Search button.
	/// Validates the selection, then starts the orbital resonance search asynchronously.
	/// </summary>
	/// <param name="sender">Event source (the button).</param>
	/// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
	/// <remarks>
	/// The search runs on a background thread. Progress is reported via the progress bar.
	...
	private async void BtnStart_Click(object sender, EventArgs e)
	{
		...
		IProgress<int> progress = new Progress<int>(value: UpdateProgress);
		try
		{
	...
	/// <summary>
	/// Processes one raw MPCORB database record and appends matching resonance results to <paramref name="results"/>.
	/// </summary>
	...
	/// </remarks>
	private static void ProcessPlanetoidLine(string line, IReadOnlyList<string> selectedPlanets, List<ResonanceResult> results)
	{
		if (line.Length < 103)
		{
			return;
