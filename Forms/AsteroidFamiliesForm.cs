...
internal partial class AsteroidFamiliesForm : BaseKryptonForm
{
	...
	private void ListView_RetrieveVirtualItem(object? sender, RetrieveVirtualItemEventArgs e)
	{
		...
		_ = item.SubItems.Add(text: p.SemiMajorAxis.ToString(format: "F4", provider: CultureInfo.CurrentCulture));
		_ = item.SubItems.Add(text: p.Eccentricity.ToString(format: "F4", provider: CultureInfo.CurrentCulture));
		_ = item.SubItems.Add(text: p.Inclination.ToString(format: "F4", provider: CultureInfo.CurrentCulture));
		_ = item.SubItems.Add(text: p.MeanAnomaly.ToString(format: "F4", provider: CultureInfo.CurrentCulture));
		_ = item.SubItems.Add(text: p.ArgPeri.ToString(format: "F4", provider: CultureInfo.CurrentCulture));
		_ = item.SubItems.Add(text: p.LongAscNode.ToString(format: "F4", provider: CultureInfo.CurrentCulture));
		e.Item = item;
	}
