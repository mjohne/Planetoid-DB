...
/// <remarks>This form computes and presents the Tisserand parameter values for a minor planet using the standard
/// three-body formula. The results are shown in a two-column table layout: planet name in the first column,
/// Tisserand parameter value in the second column.</remarks>
...
public partial class TisserandParameterOfOneMinorPlanetForm : BaseKryptonForm
{
	...
	/// <remarks>Tisserand parameter values are calculated using <see cref="TisserandParameterCalculator.CalculateTisserandParameters"/> and displayed
	...
	private void TisserandParameterOfOneMinorPlanetForm_Load(object sender, EventArgs e)
	{
		...
		try
		{
			...
			if (results.Count >= 8)
			{
				labelMercuryData.Text = results[0].TisserandValue.ToString(format: "F6");
				labelVenusData.Text = results[1].TisserandValue.ToString(format: "F6");
				labelEarthData.Text = results[2].TisserandValue.ToString(format: "F6");
				labelMarsData.Text = results[3].TisserandValue.ToString(format: "F6");
				labelJupiterData.Text = results[4].TisserandValue.ToString(format: "F6");
				labelSaturnData.Text = results[5].TisserandValue.ToString(format: "F6");
				labelUranusData.Text = results[6].TisserandValue.ToString(format: "F6");
				labelNeptuneData.Text = results[7].TisserandValue.ToString(format: "F6");
			}
		}
		catch (Exception ex)
		{
