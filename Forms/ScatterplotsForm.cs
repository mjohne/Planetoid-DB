...
public partial class ScatterplotsForm : BaseKryptonForm
{
	...
	private void ResetDisplayedResults()
	{
		...
		// Redraw the scatter plot with no data to reflect the reset state.
	...
	{
		// Update the current results list with the new scatter-plot data points.
		_currentResults = [.. results];
		// The ListView is updated in a batch to prevent flickering and improve performance while adding multiple items.
		listViewResults.BeginUpdate();
		// Clear existing items from the ListView to prepare for the new results.
		listViewResults.Items.Clear();
		// Add each scatter-plot data point as a new item in the ListView.
		foreach (ScatterPoint result in results)
		{
			// Create a new ListViewItem for the scatter-plot data point, with the main text showing the formatted X value.
			ListViewItem item = new(text: FormatNumericValue(value: result.X) + (xDefinition?.UnitSuffix ?? string.Empty))
			{
				ToolTipText = $"X: {FormatNumericValue(value: result.X)}{xDefinition?.UnitSuffix}, Y: {FormatNumericValue(value: result.Y)}{yDefinition?.UnitSuffix}"
			};
			// Add a subitem for the Y value of the scatter-plot data point.
			_ = item.SubItems.Add(text: FormatNumericValue(value: result.Y) + (yDefinition?.UnitSuffix ?? string.Empty));
			// Add the configured item to the ListView to display it in the tabular view.
			listViewResults.Items.Add(value: item);
		}
		// Redraw the scatter plot to reflect the new results.
		listViewResults.EndUpdate();
		// Update the scatter plot with the new results.
	...
	private void UpdateScatterPlot(ScatterDefinition? xDefinition, ScatterDefinition? yDefinition, IReadOnlyList<ScatterPoint> results)
	{
		...
		{
			// Extract the X and Y coordinate arrays from the data points.
			double[] xs = [.. results.Select(selector: static p => p.X)];
			double[] ys = [.. results.Select(selector: static p => p.Y)];
	...
	private async void ToolStripButtonStartCancel_Click(object? sender, EventArgs e)
	{
		...
		try
		{
			...
			Progress<List<ScatterPoint>> liveResults = new(handler: results => ApplyResults(xDefinition: xDefinition, yDefinition: yDefinition, results: results));
